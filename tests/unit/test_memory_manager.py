...
def test_delete_memory_matching_is_case_insensitive(memory_file):
    write_memory(
        memory_file,
        {"preferences": {"editor": {"value": "Google Calendar"}}},
    )
...
def test_delete_memory_normalizes_whitespace(memory_file):
    write_memory(
        memory_file,
        {"preferences": {"service": {"value": "  Google   Calendar  "}}},
    )
...
def test_delete_memory_short_non_matching_query_does_not_delete(memory_file):
    data = {
        "notes": {
            "one": {"value": "Python developer"},
            "two": {"value": "Calendar preference"},
        }
    }
    write_memory(memory_file, data)
...
def test_delete_memory_ambiguous_match_must_not_silently_delete(memory_file):
    ...
    # P1 safety contract: an ambiguous fuzzy match must not silently remove
    # an arbitrary first record. This test intentionally exposes the current
    # implementation's first-match behavior until it is fixed.
