...
def test_search_emails_formats_results():
    messages = [{
    ...
    assert "Kimdən: a@example.com" in result
    assert "Mövzu: Invoice" in result
    assert "Invoice attached" in result


def test_search_emails_empty_result():
    with patch("actions.email.search_messages", return_value=[]):
        assert search_emails("from:nobody", 5) == "Heç bir email tapılmadı."
...
def test_read_email_formats_body():
    message = {
        "id": "m1", "from": "a@example.com", "to": "b@example.com",
    ...
    assert "Mövzu: Hello" in result
    assert "Məzmun:\nHello Abdulla" in result
