...
from integrations.whatsapp.web import WhatsAppVisibleMessage
...
def test_visible_messages_are_deduplicated(tmp_path: Path):
    bridge = FakeBridge([_message("m1"), _message("m1"), _message("m2")])
...
def test_message_result_contains_structured_fields(tmp_path: Path):
    result = read_visible_whatsapp_messages(
        FakeBridge([_message("m1", "Test mesaj")]),
        tmp_path / "seen.json",
    )
