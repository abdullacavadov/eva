...
from core.proactive import NotificationPolicy, ProactiveEngine
...
def test_quiet_digest_retries_after_failed_delivery(tmp_path, monkeypatch):
    engine = ProactiveEngine(
        tmp_path / "proactive-state.json",
        NotificationPolicy(quiet_start="23:00", quiet_end="07:00"),
    )
    pending = {
        "gmail:1": _pending("gmail:1", "Birinci"),
        "gmail:2": _pending("gmail:2", "İkinci"),
    }
    monkeypatch.setattr(
        engine,
        "_collect",
        lambda: {"gmail": [], "calendar": [], "tasks": [], "memory": {}},
    )
    state = engine._load()
...
def test_digest_ack_only_removes_offered_children(tmp_path):
    ...
    state = engine._load()
    state["pending"] = {**offered, "gmail:new": _pending("gmail:new", "Yeni")}
...
def test_digest_ack_is_not_available_for_unknown_key(tmp_path):
    ...
    state = engine._load()
    state["pending"] = {"gmail:1": _pending("gmail:1", "Birinci")}
