...
def _play_audio_file(path: Path, volume: float):
    """
    Windows-da MP3-ü PowerShell System.Windows.Media.MediaPlayer ilə asinxron şəkildə səsləndirir.
    macOS-dakı 'afplay -v <vol>' davranışının qarşılığıdır: fayl bitdikdən sonra proses sonlanır, beləliklə dövrü səslər (HUD/Think) problemsiz şəkildə yenidən səsləndirilə bilir.
    Bir Popen obyekti qaytarır; dayandırmaq üçün proc.terminate() kifayətdir (PowerShell bağlandıqda səs də dayanır).
    """
...
class SoundManager:
    ...
    def play_startup(self):
        self._play_foreground(_START_FILE, tag="start",
                              loop=False, volume_factor=0.95)
    ...
    def play_error(self):
        self._play_foreground(_ERROR_FILE, tag="error",
                              loop=False, volume_factor=0.95)
...
class JarvisUI:
    ...
    def _on_configure(self, event):
        ...
        self._resize_job = self.root.after(
            80, lambda: self._resize_surface(w, h))
    ...
    def _enter_fullscreen(self):
        sw = self.root.winfo_screenwidth()
        sh = self.root.winfo_screenheight()
        # macOS native fullscreen için gerekli
    ...
    def _esc_action(self):
        """Tam ekrandadırsa, çıx; pəncərə rejimindədirsə, bağla."""
        if self._fullscreen:
            self._fullscreen = False
            self.root.attributes("-fullscreen", False)
            self.root.resizable(False, False)
    ...
    def _build_social_bar(self):
        ...
        name_lbl.bind(
            "<Button-1>", _open("https://www.instagram.com/a.javad_off"))

        self._icon_ig = _load_icon("instagram-logo.png")
        self._icon_in = _load_icon("linkedin-logo.png")

        if self._icon_ig:
            ig_lbl = tk.Label(bar, image=self._icon_ig,
        ...
        if self._icon_in:
            yt_lbl = tk.Label(bar, image=self._icon_in,
                              bg=C_BG, cursor="hand2")
            yt_lbl.pack(side="left", padx=4)
            yt_lbl.bind(
                "<Button-1>", _open("https://www.linkedin.com/in/abdulla-javadov-4a3740a7/"))
    ...
    def _build_settings_panel(self):
        ...
        self._settings_btn_canvas.bind(
            "<Button-1>", lambda e: self._toggle_settings_panel())
        ...
        self._settings_tab_settings.bind(
            "<Button-1>", lambda e: self._set_settings_tab("settings"))
        ...
        self._settings_tab_debug.bind(
            "<Button-1>", lambda e: self._set_settings_tab("debug"))
    ...
    def _draw_settings_button(self):
        ...
        c.create_text(14, 15, text="TƏNZİMLƏMƏLƏR", fill=C_PRI,
                      font=font_display(10), anchor="w")
        c.create_text(14, 33, text=MODEL_BADGE, fill="#4f7b78",
    ...
    def _draw_settings_tabs(self):
        for key, canvas, label in (
            ...
            bl = 7
            for bx, by, sx, sy in [(0, 0, 1, 1), (bw, 0, -1, 1), (0, bh, 1, -1), (bw, bh, -1, -1)]:
                canvas.create_line(bx, by, bx + sx * bl, by,
                                   fill=outline, width=1)
                canvas.create_line(bx, by, bx, by + sy * bl,
                                   fill=outline, width=1)
            canvas.create_text(bw // 2, bh // 2, text=label,
                               fill=text_col, font=font_body_bold(9))
    ...
    def _layout_settings_controls(self):
        ...
        self._sfx_canvas.place(
            x=inner_w - int(self._sfx_canvas["width"]) - 4, y=0)
    ...
    def _render_debug_logs(self):
        ...
        if not self._debug_entries:
            self._debug_text.insert(
                tk.END, "Henüz not edilebilir hata yok.\n", "info")
        else:
    ...
    def _build_api_button(self, parent=None):
        ...
        self._api_canvas.bind(
            "<Button-1>", lambda e: self._open_api_settings())
...
"""
    ...
    def _build_autostart_button(self, parent=None):
        ...
        self._autostart_canvas.bind(
            "<Button-1>", lambda e: self._toggle_autostart())
    ...
    def _draw_autostart_button(self):
        ...
        icon = "◉" if on else "○"
        text = f"{icon}  AÇILIŞDA BAŞLAT  {'[aktiv]' if on else '[deaktiv]'}"
    ...
    def _toggle_autostart(self):
        try:
            ...
            if self._is_autostart_installed():
                ...
                self.write_log("SYS: Avtomatik başlatma deaktiv edildi.")
            else:
                create_startup_shortcut()
                self.write_log(
                    "SYS: Avtomatik başlatma aktiv edildi. Windows açıldıqda VICTOR aktivləşir.")
    ...
    def _build_shortcut_button(self, parent=None):
        ...
        self._shortcut_canvas.bind(
            "<Button-1>", lambda e: self._create_desktop_shortcut())
    ...
    def _draw_shortcut_button(self, state: str = "idle"):
        ...
        if state == "ok":
            col, icon, label = C_GREEN, "✓", "Masaüstü qısayol yaradıldı"
        elif state == "err":
            col, icon, label = C_RED, "✕", "Xəta: qısayol yaradılmadı"
        else:
            col, icon, label = C_MID, "⊞", "Masaüstünə qısayol əlavə et"
    ...
    def _draw_sfx_button(self):
        ...
        c.create_text(BW//2, BH//2, text=text,
    ...
    def _on_voice_select(self, voice: str):
        ...
        if self.on_voice_change:
            threading.Thread(target=self.on_voice_change,
                             args=(voice,), daemon=True).start()
    ...
    def _toggle_pause(self):
        ...
        if self.paused:
            ...
            self.write_log("SYS: VICTOR dayandırıldı.")
        else:
            self.set_state("THINKING")
            self.write_log("SYS: VICTOR davam edir...")
        self._sync_sound_state()
        if self.on_pause_toggle:
            threading.Thread(target=self.on_pause_toggle,
                             args=(self.paused,), daemon=True).start()

    def _shutdown(self):
        self.sound.stop_all()
        self.write_log("SYS: VICTOR bağlanır...")
    ...
    def _place_layout_widgets(self):
        ...
        self._cam_canvas.place(x=start_x + mute_w +
                               pause_w + gap * 2, y=row1_y)
        self._shutdown_canvas.place(
            x=start_x + mute_w + pause_w + cam_w + gap * 3, y=row1_y)
        ...
        if self._settings_open:
            ...
            if self._settings_tab == "debug":
                ...
                self._debug_body.place(
                    x=12, y=76, width=panel_w - 24, height=panel_h - 88)
                self._debug_text.place(
                    x=0, y=0, width=panel_w - 24, height=panel_h - 88)
                self._debug_body.lift()
            else:
                self._debug_body.place_forget()
                self._settings_body.place(
                    x=12, y=76, width=panel_w - 24, height=panel_h - 88)
        ...
        self._input_entry.place(
            x=self.CHAT_X, y=self.CHAT_INPUT_Y, width=inp_w, height=INPUT_H)
        self._send_btn.place(x=self.CHAT_X + inp_w + 8,
                             y=self.CHAT_INPUT_Y, width=76, height=INPUT_H)
    ...
    def _on_input_submit(self, event=None):
        ...
        if self.paused:
            self.write_log(
                "SYS: VICTOR fasilədədir. Davam etmək üçün 'Fasilə' rejimini deaktiv et.")
            return
        self._input_var.set("")
        if text.lower() in ("sus", "dur", "stop", "sessiz", "kes"):
            self.write_log("SYS: ⏹ Səs kəsildi.")
    ...
    def wake_up(self):
        ...
        def _do():
            try:
                ...
                self.root.after(
                    3000, lambda: self.root.attributes("-topmost", False))
            except Exception:
                pass
    ...
    def _start_typing(self):
        ...
        if tl.startswith("Siz:") or tl.startswith("you:"):
            tag = "you"
        elif tl.startswith("VICTOR:") or tl.startswith("ai:"):
            tag = "ai"
        elif tl.startswith("Err:") or "error" in tl:
            tag = "err"
        else:
    ...
    def _update_stats(self):
        try:
            ...
            if dt > 0:
                self._stats['net_up'] = max(
                    0, (net.bytes_sent - self._last_net.bytes_sent) / dt / 1024)
                self._stats['net_down'] = max(
                    0, (net.bytes_recv - self._last_net.bytes_recv) / dt / 1024)
            self._last_net = net
    ...
    def _animate(self):
        ...
        self._cam_orb_shift += (self._cam_orb_shift_target -
                                self._cam_orb_shift) * _CE
        self._cam_orb_face += (self._cam_orb_face_target -
                               self._cam_orb_face) * _CE
    ...
    def _parse_weather_card(self, text: str) -> dict:
        if not text or "alınmadı" in text.lower() or "alınmadi" in text.lower():
            return {
                "city": "Bakı",
                "primary": "--°",
                "condition": "Hava məlumatı yoxdur",
                "feels_like": "--",
                "humidity": "--",
                "pressure": "--",
                "wind": "--",
        ...
        prefix = " üçün hava durumu:"
        city = "Bakı"

        if prefix in text:
            city, text = text.split(prefix, 1)
            city = city.strip()

        parts = [p.strip() for p in text.rstrip(".").split(",")]
        ...
        for part in parts:
            ...
            if "dərəcə" in lower and "hiss edilən" not in lower:
                if primary == "--°":
                    primary = part.replace(" dərəcə", "°")

            elif "hiss edilən" in lower:
                feels_like = part.replace("hiss edilən ", "")

            elif "rütubət" in lower:
                humidity = part.replace("nisbi rütubət ", "")

            elif "təzyiq" in lower:
                pressure = part.replace("atmosfer təzyiqi ", "")

            elif "külək" in lower:
                wind = part.replace("külək ", "")

            else:
                if not condition:
                    condition = part

        # Hava şəraitinə uyğun sadə emoji icon
        icon = self._weather_icon(condition)

        details = [
            f"Hiss edilən: {feels_like}",
            f"Rütubət: {humidity}",
            f"Təzyiq: {pressure}",
            f"Külək: {wind}",
        ]
    ...
    def _refresh_brief_cards(self):
        try:
            self._weather_card = get_weather_summary("Baku")
            ...
            self._weather_card = {
                ...
                "wind_direction": "--",
                "weather_code": None,
                "condition": "Hava məlumatı alınmadı",
            }
    ...
    def _bar(self, c, x, y, w, h, pct, color):
        c.create_rectangle(x, y, x+w, y+h, fill="#061212",
    ...
    def _sparkline(self, c, x, y, w, h, data):
        c.create_rectangle(x, y, x+w, y+h, fill="#050e0e",
                           outline=C_DIM, width=1)
        n = len(data)
        if n < 2:
            return
    ...
    def _draw_info_card(self, c, x0, y0, pw, ph, title, accent=C_PRI):
        ...
        border = accent if focus > 0.08 else (
            "#35504d" if dimmed else self._ac(0, 120, 112, 190))
        ...
        if focus > 0.08:
            for inset in range(3):
                c.create_rectangle(
                    ...
                    outline=self._ac(
                        *ORB_COLORS["LISTENING"], max(12, glow - inset * 28)),
                    width=1,
                )
    ...
    def _draw_left_panel(self, c):
        ...
        cards = [
            ("time", 0.28, "VAXT", C_GOLD),
            ("weather", 0.26, "HAVA · BAKI", C_BLUE),
            ("system", 0.46, "SİSTEM VƏZİYYƏTİ", C_PRI),
        ]
        any_focus_active = bool(self._panel_focus) and (
            self._panel_focus_until > time.time())
        weights = []
        for section, weight, _, _ in cards:
            weights.append(
                weight + (0.12 if self._focus_boost_for(section) > 0.08 else 0.0))
        total_weight = sum(weights)
        heights = [int(card_area_h * (weight / total_weight))
        ...
        for (section, _, title, accent), ph in zip(cards, heights):
            ...
            self._draw_info_card(c, section_x, current_y, section_pw,
                                 ph, title, accent=accent if not dimmed else "#72807f")

            if section == "time":
                time_x = section_x + section_pad
                time_y = current_y + 64
                ...
                c.create_text(
                    ...
                    text=time_text,
                    fill=muted_primary,
                    font=font_display(36 if focus_boost > 0.08 else 34),
                    anchor="w"
                )

                bbox = c.bbox(c.create_text(
                    -1000,
                    -1000,
                    text=time_text,
                    font=font_display(36 if focus_boost > 0.08 else 34)
                ...
                c.create_text(
                    time_x,
                    current_y + 118,
                    text=time.strftime("%d %B %Y").upper(),
                    fill=muted_gold,
                ...
                if weather.get("success"):
                    ...
                    c.create_text(
                        section_x + section_pad,
                        current_y + 58,
                        text=f"{temperature:g}°" if temperature is not None else "--°",
                        fill=muted_primary,
                        font=font_display(30 if focus_boost > 0.08 else 28),
                        anchor="w"
                    )
                    ...
                    c.create_text(
                        section_x + section_pad,
                        current_y + 84,
                        text=weather.get("city", "Bakı").upper(),
                        fill=muted_label,
                        font=font_body_bold(10),
                        anchor="w"
                    )
                    ...
                    details = [
                        f"Hiss edilən: {weather.get('feels_like')}°C",
                        f"Rütubət: {weather.get('humidity')}%",
                        f"Təzyiq: {weather.get('pressure')} hPa",
                        (
                            f"Külək: {weather.get('wind_speed')} km/saat, "
                ...
                for label, key, unit in [("CPU", "cpu", "%"), ("RAM", "ram", "%"), ("DISK", "disk", "%"), ("BATTERY", "battery", "%")]:
                    val = self._stats[key]
                    col = C_RED if val > 80 and key != "battery" else C_ORG if val > 55 and key != "battery" else (
                        C_RED if key == "battery" and val < 20 else C_GREEN if key == "battery" else C_PRI)
                    if dimmed:
                        col = muted_red if col == C_RED else muted_warn if col == C_ORG else muted_green if col == C_GREEN else muted_primary
                    c.create_text(section_x+section_pad, cy, text=label,
                                  fill=muted_label, font=font_body(10), anchor="w")
                    c.create_text(section_x+section_pw-section_pad, cy,
                                  text=f"{val:.0f}{unit}", fill=col, font=font_body_bold(10), anchor="e")
                    cy += 14
                    self._bar(c, section_x+section_pad,
                              cy, section_bw, 7, val, col)
                    cy += 16
                ...
                c.create_line(section_x+section_pad, cy-4, section_x+section_pw -
                              section_pad, cy-4, fill="#173130" if dimmed else C_DIM)
                c.create_text(section_x+section_pad, cy+10,
                              text=f"▲ {up_s}", fill=muted_warn, font=font_body(10), anchor="w")
                c.create_text(section_x+section_pw-section_pad, cy+10,
                              text=f"▼ {down_s}", fill=muted_green, font=font_body(10), anchor="e")
    ...
    def _draw_right_panel(self, c):
        ...
        c.create_text(x0+14, y0+16, text="SÖHBƏT", fill=C_PRI,
    ...
    def _draw_orb(self, c, cam: bool = False):
        ...
        speak_pulse = 1.0
        if self.speaking:
            speak_pulse = 1.0 + 0.12 * \
                math.sin(t * 0.23) + 0.05 * math.sin(t * 0.11 + 1.2)
        elif self.user_speaking:
        ...
        base_face = (int(self._cam_orb_face)
                     if self._cam_orb_face > 1.0 else self.FACE)
        ...
        if not cam:
            for frac, width, alpha_mult in (
                ...
                c.create_oval(
                    FCX-rr, FCY-rr, FCX+rr, FCY+rr,
                    outline=self._ac(R, G, B, int(
                        ha * alpha_mult * (0.4 if self.paused else 1.0))),
                    width=width,
                )
        ...
        for idx, sp in enumerate(self.orb_shell_particles):
            angle = sp['angle'] + t * sp['speed'] * \
                (2.8 if self.speaking else 1.6 if self.user_speaking else 1.1)
            wobble = 1.0 + (0.07 if self.speaking else 0.035) * \
                math.sin(t * 0.08 + sp['phase'])
            x = FCX + math.cos(angle) * shell_r * wobble
            y = FCY + math.sin(angle) * shell_r * wobble
            alpha = int(
                (70 + 120 * sp['glow']) * (0.26 if self.paused else 0.52 + activity * 0.45))
            if idx % 9 == 0 and not self.paused:
                col = self._ac(
                    accent_rgb[0], accent_rgb[1], accent_rgb[2], min(255, alpha + 30))
            else:
        ...
        if not cam:
            ...
            for start, extent, width, accent in (
                ...
                ((self.rings_spin[2] + 28) % 360,
                 64 if self.user_speaking else 40, 3, False),
                ((self.rings_spin[2] + 212) % 360, 18, 2, True),
            ):
                rr = arc_r1 if width == 3 else arc_r2
                if accent and not self.paused:
                    col = self._ac(
                        accent_rgb[0], accent_rgb[1], accent_rgb[2], int(120 + 80 * activity))
                else:
                    col = self._ac(R, G, B, int(
                        ha * (1.2 if width == 3 else 0.7)))
                c.create_arc(
                    FCX-rr, FCY-rr, FCX+rr, FCY+rr,
        ...
        for idx, p in enumerate(self.orb_particles):
            ...
            wobble = 1.0 + (0.30 if self.speaking else 0.18) * \
                math.sin(t * p['wobble'] + p['phase'])
            orbit = field_limit * p['orbit'] * wobble
            depth = 0.5 + 0.5 * math.sin(angle * 2.0 + t * 0.013 + p['phase'])
            y_squash = 0.62 + depth * 0.38
            drift = (
                8.0 if self.speaking else 5.0 if self.user_speaking else 4.0) * p['depth']
            x = FCX + math.cos(angle) * orbit + \
                math.sin(t * 0.011 + p['phase']) * drift
            y = FCY + math.sin(angle) * orbit * y_squash + \
                math.cos(t * 0.010 + p['phase']) * drift
            base_alpha = int(
                (18 + 155 * p['depth']) * (0.24 + activity * 0.86) * (0.45 + depth * 0.75))
            if self.paused:
                base_alpha = int(base_alpha * 0.40)
            if idx % 11 == 0 and not self.paused:
                col = self._ac(accent_rgb[0], accent_rgb[1], accent_rgb[2], min(
                    255, base_alpha + 25))
            elif self.user_speaking and idx % 7 == 0:
                col = self._ac(120, 205, 255, min(255, base_alpha + 20))
            else:
                col = self._ac(R, G, B, base_alpha)
            pr = p['size'] * (0.70 if self.paused else 0.90 +
                              depth * 0.65 + 0.30 * activity * p['depth'])
            c.create_oval(x-pr, y-pr, x+pr, y+pr, fill=col, outline="")
            if idx % 18 == 0 and not self.paused:
                c.create_line(
                    ...
                    FCY + (y-FCY) * 0.18,
                    x, y,
                    fill=self._ac(R, G, B, int(
                        18 + 35 * p['depth'] * activity)),
                    width=1,
                )
    ...
    def _draw(self):
        ...
        if t % 3 == 0:
            ...
            for x in range(0, W, step):
                for y in range(0, H, step):
                    c.create_rectangle(
                        x, y, x+1, y+1, fill=C_DIMMER, outline="")
        ...
        c.create_text(W//2, 52, text="Enhanced Virtual Assistant",
        ...
        c.create_text(W//2, H-13, fill=C_DIM, font=font_body(9),
                      text="VICTOR · Windows Edition · Realtime Voice Core")
    ...
    def _show_setup_ui(self, edit_mode: bool = False):
        ...
        title = "◈ API Parametrləri" if edit_mode else "◈ **İlkin quraşdırma tələb olunur**"
        subtitle = (
            "Gemini API açarınızı yeniləyin."
            if edit_mode else
            "Gemini API açarınızı daxil edin."
        )
        ...
        tk.Button(buttons, text="▸ Yadda saxla",
    ...
    def _save_api_key(self):
        ...
        youtube_handle = self.youtube_handle_entry.get(
        ).strip() if self.youtube_handle_entry else ""
        save_app_config(
            {
                "gemini_api_key": key,
        ...
        if was_ready:
            ...
            self.set_state("LISTENING")
            self.write_log("SYS: VICTOR hazırdır. Dinləyirəm...")
