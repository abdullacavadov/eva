...
export function EvaOrb({ state }: { state: EvaState }) {
  ...
  const orbStyle = {
    ...
    '--orb-strong-glow': withAlpha(color, '0.72'),
  } as CSSProperties
  ...
  return (
    <section className={`eva-core state-${state.toLowerCase()}`} aria-label={`EVA ${stateLabel[state]}`} style={orbStyle}>
      ...
      <div className="orb-glow" style={{ background: glow, boxShadow: `0 0 70px ${softGlow}` }} />
      <div className="orb-sphere" style={{ borderColor: withAlpha(color, '0.58'), background: `radial-gradient(circle at 42% 36%, ${withAlpha(color, '0.20')}, rgba(3, 26, 39, .72) 43%, rgba(1, 8, 13, .95) 70%)`, boxShadow: `0 0 36px ${softGlow}, inset 0 0 45px ${softGlow}` }}>
        ...
        <span className="orb-wordmark" style={{ color: '#D8F7FF', textShadow: `0 0 14px ${color}, 0 0 38px ${withAlpha(color, '0.62')}` }}>E.V.A</span>
      </div>
      <div className="orb-beam" style={{ background: `linear-gradient(transparent, ${withAlpha(color, '0.62')}, transparent)`, boxShadow: `0 0 30px 5px ${softGlow}` }} />
      <div className="orb-platform" style={{ borderColor: withAlpha(color, '0.34'), boxShadow: `0 0 22px ${softGlow}, inset 0 0 22px ${softGlow}` }}>
      ...
      <div className="orb-waveform" aria-hidden="true">
        {Array.from({ length: 28 }, (_, index) => (
          <i key={index} style={{ '--bar': `${16 + ((index * 17) % 54)}%`, background: color, boxShadow: `0 0 8px ${withAlpha(color, '0.70')}` } as CSSProperties} />
        ))}
      </div>
    </section>
  )
