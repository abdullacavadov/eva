...
export function ContextPanel({ context }: { context: EvaContext }) {
  return (
    <section className="panel context-panel">
      <div className="panel-heading"><span>ACTIVE CONTEXT</span><small>{context.source ?? 'EVA'}</small></div>
      <div className="context-title">{context.title ?? 'No active context'}</div>
      ...
      <div className="context-footer"><span>{context.items.length} items</span><span>CONTEXT READY</span></div>
    </section>
  )
}
