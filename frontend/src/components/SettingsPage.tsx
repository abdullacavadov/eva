    ...
    <div className="settings-screen">
      ...
      <header className="settings-header">
        ...
        <div>
          ...
          <h1>PARAMETRLƏR</h1>
          <p>EVA-nın davranışını, səsini və inteqrasiyalarını idarə et.</p>
        </div>
      ...
      <main className="settings-content">
        ...
        <section className="settings-section">
          ...
          <div className="settings-control-row">
            <div>
              <strong>SFX səsləri</strong>
              <small>HUD, startup, thinking, success və error səsləri</small>
            </div>
            <button
              className={`toggle ${settings.sfx_enabled ? 'on' : ''}`}
              onClick={() => update('sfx_enabled', !settings.sfx_enabled)}
            >
              <span />
            </button>
          </div>
          <label className="settings-range">
            <div>
              <span>SFX səs səviyyəsi</span>
              <strong>{sfxLabel}</strong>
            </div>
            <input
              type="range"
              min="0"
              ...
              onChange={(e) => update('sfx_volume', Number(e.target.value))}
            />
          </label>
        </section>
        <section className="settings-section">
          <div className="settings-section-title">
            ...
            <span>EVA DAVRANIŞI</span>
            <small>RUNTIME</small>
          </div>
          <div className="settings-toggle-grid">
            <label className="settings-control-row compact">
              <div>
                <strong>Proaktiv bildirişlər</strong>
                <small>EVA özü bildiriş yarada bilər</small>
              </div>
              <button
                className={`toggle ${settings.proactive_enabled ? 'on' : ''}`}
                onClick={() =>
                  update('proactive_enabled', !settings.proactive_enabled)
                }
              >
                <span />
              </button>
            ...
            <label className="settings-control-row compact">
              <div>
                <strong>Wake listener</strong>
                <small>Wake gesture/listener aktiv olsun</small>
              </div>
              <button
                className={`toggle ${settings.wake_listener_enabled ? 'on' : ''}`}
                onClick={() =>
                  update(
                    'wake_listener_enabled',
                    !settings.wake_listener_enabled
                  )
                }
              >
                <span />
            ...
            <label className="settings-control-row compact">
              <div>
                <strong>Avtomatik başlatma</strong>
                <small>Sistem açılışında EVA-nı işə sal</small>
              </div>
              <button
                className={`toggle ${settings.auto_start ? 'on' : ''}`}
        ...
        <section className="settings-section">
          <div className="settings-section-title">
            ...
            <span>ŞƏXSİLƏŞDİRMƏ</span>
            <small>PROFILE</small>
          </div>
          <div className="settings-info">
            Bu bölmə genişləndiriləcək: istifadəçi adı, cavab üslubu,
            proaktivlik həddi, orb davranışı və digər UI parametrləri burada
            mərkəzləşdiriləcək.
          </div>
        </section>
      </main>
