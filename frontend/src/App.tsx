...
import { faTemperatureLow, faWind, faDroplet, faGaugeHigh, faCloudSun, faMicrochip, faCalendarDays, faListCheck, faBell, faEnvelope } from '@fortawesome/free-solid-svg-icons'
...
export default function App() {
  ...
  const applyEvent = (event: EvaEvent) => {
    ...
    if (event.type === 'bridge.error') { setOnline(false); setState('ERROR'); setActivities((items) => [...items, { id: crypto.randomUUID(), time: new Date().toLocaleTimeString('az-AZ'), text: 'UI bağlantı xətası', kind: 'error', detail: event.message }]) }
    ...
    if (event.type === 'tool.started') { setState('EXECUTING'); setActivities((items) => [...items, { id: crypto.randomUUID(), time: new Date().toLocaleTimeString('az-AZ'), text: `Alət icra olunur: ${event.tool}`, kind: 'action' }]) }
    if (event.type === 'tool.completed') setActivities((items) => [...items, { id: crypto.randomUUID(), time: new Date().toLocaleTimeString('az-AZ'), text: event.success ? `Alət tamamlandı: ${event.tool}` : `Alət uğursuz oldu: ${event.tool}`, kind: event.success ? 'success' : 'error', detail: event.result }])
  }

  const { connected, sendText } = useEvaConnection(applyEvent)
  useEffect(() => setOnline(connected), [connected])
  useEffect(() => { const timer = window.setInterval(() => setNow(new Date()), 1000); return () => window.clearInterval(timer) }, [])
  useEffect(() => { let cancelled = false; const load = async () => { const data = await fetchDashboard(); if (cancelled) return; setDashboard(data); if (data.context.items.length > 0) setContext(data.context) }; void load(); const timer = window.setInterval(() => void load(), 15000); return () => { cancelled = true; window.clearInterval(timer) } }, [])

  const statusText = useMemo(() => stateStatusLabel[state], [state])
  const { overview, weather, system } = dashboard
  const handleSubmit = (text: string) => { if (!sendText(text)) { setState('ERROR'); setActivities((items) => [...items, { id: crypto.randomUUID(), time: new Date().toLocaleTimeString('az-AZ'), text: 'EVA bağlantısı hazır deyil', kind: 'error', detail: 'WebSocket bağlantısı gözlənilir.' }]) } }
  ...
  return <div className="app-shell">
    <div className="ambient ambient-a" /><div className="ambient ambient-b" /><Sidebar />
    <main className="workspace">
      ...
      <div className="dashboard-grid">
        ...
        <section className="core-column"><div className="core-meta"><span>SÜNİ İNTELLEKT NÜVƏSİ / HOLOQRAFİK İNTERFEYS</span><span>v6.1</span></div><EvaOrb state={state} /><ConversationPanel messages={messages} onSubmit={handleSubmit} /></section>
        <div className="right-stack"><ContextPanel context={context} /><ActivityFeed items={activities} /></div>
      </div>
    </main>
  </div>
}
