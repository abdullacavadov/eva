"""VICTOR-nın avtonom YouTube video prodakşn pipeline-ı."""
...
def _inside_media(path: str | Path) -> Path:
    ...
    try:
        ...
        raise ValueError("Media faylı yalnız VICTOR media qovluğunda ola bilər.") from exc
    return candidate
...
def _plan(brief: str, contact_sheet: Path | None, asset_labels: list[str], audio_labels: list[str]) -> dict:
    ...
    prompt = f"""Sən VICTOR üçün peşəkar YouTube video prodüserisən. İstifadəçinin tələbi:
{brief}
