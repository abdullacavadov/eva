...
def _deletion_candidates(plan: QueryPlan, limit: int) -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    if "calendar" in plan.sources:
        try:
            ...
            for item in result.get("data", []):
                if not entity or _entity_matches(item, entity) or _matches_search_terms(item, plan.search_terms):
                    candidates.append(normalize_item("calendar", item, result.get("type", "calendar_event")))
    ...
    if "tasks" in plan.sources:
        try:
            ...
            for item in result.get("data", []):
                if not entity or _entity_matches(item, entity) or _matches_search_terms(item, plan.search_terms):
                    candidates.append(normalize_item("tasks", item, result.get("type", "task")))
            if result.get("status") == "error":
    ...
    if "memory" in plan.sources:
        ...
        for term in terms:
            for item in _memory_items(term, plan.period):
                memory_items.setdefault(item["id"], item)
        for item in memory_items.values():
            if not entity or _entity_matches(item, entity) or _matches_search_terms(item, plan.search_terms):
                candidates.append(normalize_item("memory", item, "memory"))
...
def _execute_daily_report(limit: int) -> dict[str, Any]:
    ...
    try:
        results["gmail"] = search_emails(f"after:{target_date.replace('-', '/')} before:{(datetime.fromisoformat(target_date) + timedelta(days=1)).strftime('%Y/%m/%d')}", limit)
    except Exception as exc:
...
def execute_unified_query(query: str, limit: int = 8) -> dict[str, Any]:
    ...
    if plan.intent == "deletion":
        ...
        if not candidates:
            return make_unified_result(query, plan.intent, list(plan.sources), [], errors, {"period": plan.period, "search_text": plan.search_text, "search_terms": list(plan.search_terms), "requires_confirmation": True, "source_count": len(plan.sources), "entity": plan.metadata.get("entity", ""), "deletion_safe": True, "message": "Silinəcək uyğun qeyd tapılmadı."})
        return make_unified_result(query, plan.intent, list(plan.sources), candidates, errors, {"period": plan.period, "search_text": plan.search_text, "search_terms": list(plan.search_terms), "requires_confirmation": True, "source_count": len(plan.sources), "entity": plan.metadata.get("entity", ""), "deletion_safe": True, "message": "Silmə əməliyyatı icra edilmədi. Əvvəl mənbəni və konkret qeydi dəqiqləşdir."})
    ...
    for source in execution_sources:
        try:
            if source == "calendar":
                ...
                from actions.reminders import get_reminders
                result = get_reminders(plan.period if plan.period else "upcoming", max(limit, 20) if entity else limit, "")
                _append_source(items, source, result, limit, entity, plan.search_terms)
