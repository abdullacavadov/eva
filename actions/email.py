...
def _structured_message(message: dict[str, str], include_body: bool = False) -> dict:
    return {
        "id": f"email:{message.get('id', '')}", "gmail_message_id": message.get("id", ""),
        "thread_id": message.get("thread_id", ""), "from": message.get("from", ""), "to": message.get("to", ""),
        "cc": message.get("cc", ""), "bcc": message.get("bcc", ""), "subject": message.get("subject", ""),
        "date": message.get("date", ""), "snippet": message.get("snippet", ""),
        "message_id_header": message.get("message_id_header", ""), "references": message.get("references", ""),
        "in_reply_to": message.get("in_reply_to", ""), **({"body": message.get("body", "")} if include_body else {}),
    }


def _combine_query(query: str, folder: str) -> str:
    folder_filter = folder_query(folder); query = str(query or "").strip()
    if folder_filter and query: return f"{folder_filter} {query}"
    return folder_filter or query


def search_emails(query: str = "", limit: int = 10, folder: str = "") -> dict:
    try:
        effective_query = _combine_query(query, folder); result = search_messages(query=effective_query, limit=limit)
        messages = result["messages"]; payload = {"query": query, "folder": folder, "limit": limit}
        ...
        return success("email", [_structured_message(message) for message in messages], payload,
                       {"returned_count": result["returned_count"], "has_more": result["has_more"], "effective_query": effective_query}, count=result["count"])
    except Exception as exc:
        return error("email", str(exc), {"query": query, "folder": folder, "limit": limit})
...
def prepare_trash_emails(folder: str = "", message_id: str = "", query: str = "") -> dict:
    try:
        ...
        if message_id:
            message_ids = [message_id]; description = "seçilmiş email"; effective_query = ""
        else:
            ...
            message_ids = list_message_ids(effective_query); description = folder or effective_query
        ...
        return success("email", [{"id": f"email:{message_id}" if message_id else f"email:trash:{confirmation_id}", "action": "trash", "target": description, "target_count": len(message_ids), "status": "pending_confirmation"}],
                       {"folder": folder, "message_id": message_id, "query": query},
                       {"requires_confirmation": True, "confirmation_action": "trash_emails", "confirmation_id": confirmation_id,
                        "confirmation_message": f"{description} üçün {len(message_ids)} email Trash-a göndəriləcək. Təsdiq edirsən?"}, count=len(message_ids))
    except Exception as exc:
        return error("email", str(exc), {"folder": folder, "message_id": message_id, "query": query})
...
def trash_emails(confirmation_id: str = "") -> dict:
    ...
    try:
        ...
        for message_id in plan["message_ids"]:
            ...
            trashed_items.append({"id": f"email:{trashed['message_id']}", "gmail_message_id": trashed["message_id"], "thread_id": trashed["thread_id"], "action": "trash", "status": "trashed"})
...
def prepare_email_reply(message_id: str, body: str) -> dict:
    ...
    try:
        ...
        return success("email", [item], {"action": "reply", "message_id": message_id}, {"selected_id": item["id"], "requires_confirmation": True, "confirmation_action": "send_email", "confirmation_id": confirmation_id})
...
def prepare_new_email(to: str, subject: str, body: str, cc: str = "", bcc: str = "") -> dict:
    try:
        ...
        item = {"id": f"email:draft:{draft['draft_id']}", "draft_id": draft["draft_id"], "gmail_message_id": draft["gmail_message_id"], "thread_id": draft["thread_id"], "to": to, "cc": cc, "bcc": bcc, "subject": subject, "body": body, "action": "new", "status": "draft"}
        confirmation_id = issue_confirmation("send_email", {"draft_id": draft["draft_id"]})
        return success("email", [item], {"action": "new", "to": to, "subject": subject}, {"selected_id": item["id"], "requires_confirmation": True, "confirmation_action": "send_email", "confirmation_id": confirmation_id})
...
def send_email(draft_id: str, confirmation_id: str = "") -> dict:
    draft_id = str(draft_id or "").strip(); confirmation_id = str(confirmation_id or "").strip()
    if not draft_id: raise ValueError("Email draft_id tələb olunur.")
    if not confirmation_id:
        return {"type": "email", "status": "needs_confirmation", "data": [], "count": 0, "meta": {"requires_confirmation": True, "confirmation_action": "send_email", "message": "Email göndərmək üçün açıq təsdiq tələb olunur.", "draft_id": draft_id}}
    try:
        consume_confirmation(confirmation_id, "send_email", {"draft_id": draft_id})
        sent = send_draft(draft_id)
        item = {"id": f"email:sent:{sent['message_id']}", "gmail_message_id": sent["message_id"], "thread_id": sent["thread_id"], "draft_id": draft_id, "action": "send", "status": "sent"}
...
def prepare_email_deletion(scope: str, draft_id: str = "") -> dict:
    scope = str(scope or "").strip().lower(); draft_id = str(draft_id or "").strip()
    try:
        draft_ids, message_ids = _email_delete_target(scope, draft_id); target_count = len(draft_ids) + len(message_ids); confirmation_id = uuid.uuid4().hex
...
def delete_email(confirmation_id: str) -> dict:
    ...
    try:
        deleted_drafts = delete_drafts(plan["draft_ids"]); deleted_messages = batch_delete_messages(plan["message_ids"]); deleted_count = deleted_drafts + deleted_messages
        item = {"id": f"email:delete:{confirmation_id}", "action": "delete", "scope": plan["scope"], "deleted_count": deleted_count, "permanent": True, "status": "deleted"}
