...
def _make_prompt(query: str) -> str:
    ...
    return (
        "Sen JARVIS'in görmə modulusan. Veb-kameradan gələn canlı kadrı analiz edirsən.\n\n"
        f"İstifadəçinin sualı: {q}\n\n"
        "Gördüklərini Azərbaycan dilində aydın və qısa şəkildə izah et.\n"
        "- Obyekt tanıma aparılırsa, tam adı ilə qeyd et.\n"
        "- Marka, rəng, forma kimi fərqləndirici xüsusiyyətləri sadala.\n"
        "- Əmin olmadığın şeyləri 'böyük ehtimalla' və ya '... kimi görünür' ifadələri ilə bildir.\n"
...
def _analyze(query: str, image_path: Path) -> str:
    ...
    if not api_key:
        return "Gemini API key əksikdi — kamera analizi işə düşmür."
    ...
    for model in VISION_MODELS:
        for attempt, delay in enumerate(delays, start=1):
            try:
                ...
                if text:
                    return text
                raise RuntimeError("Gemini keçərli bir yanıt döndürmedi.")
            except Exception as exc:
                last_exc = exc
